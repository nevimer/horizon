...
/client/verb/SetWindowIconSize(val as num)
	set hidden = 0
	if(prefs && val != winget(src, "mapwindow.map", "icon-size"))
...
/client/verb/OnResize()
	...
	var/divisor = text2num(winget(src, "mapwindow.map", "icon-size")) || prefs.pixel_size || world.icon_size
	...
	change_view(getScreenSize(!prefs?.widescreenpref))
