...
/obj/machinery/computer/teleporter/proc/set_target(mob/user)
	...
	if(regime_set == "Teleporter")
		...
		var/desc = tgui_input_list(user, "Please select a location to lock in.", "Locking Computer", sortList(L))
		...
		if(!L.len)
			...
			return
		var/desc = tgui_input_list(user, "Please select a station to lock in.", "Locking Computer", sortList(L))
